...
def scrape_jobs(keyword: str = Query(..., description="検索キーワード（例: Python）")):
    ...
    try:
        ...
        # 【ポートフォリオ用デモ設定】
        # 本来はここで 504 Gateway Timeout を返して処理を中断しますが、
        # 今回はデモ画面でCSVダウンロード機能を見せるため、エラーを無視してダミーデータを生成します。
        # raise HTTPException(status_code=504, detail="Target website timeout. Please try again later.")
        pass
    except requests.exceptions.RequestException as e:
        # 【ポートフォリオ用デモ設定】
        # その他のネットワークエラー（存在しないURLなど）時、本来は 502 Bad Gateway を返しますが、
        # デモ目的のためエラーを無視します。（コンソールログには記録）
        print(f"Scraping error (ignored for demo): {e}")
        # raise HTTPException(status_code=502, detail=f"Bad Gateway: Failed to fetch data from target website. Error: {str(e)}")
        pass

    # HTMLのパース（エラーでresponseが存在しない場合は空文字を使用）
